...
def grad_encrypt(xx, S1a, S2a, S3a, S4a, S1m, S2m, S3m, S4m):
    ...
    W1 = 2 * c11 * x1
    x_1 = W1
    x_1_hat = Integrization(x_1, r2, omega)
    ...
    # this part is done by node 1 becuse ut includes the private data
    W2_coeffieicent = c12
    # x_1_hat = Integrization(x_1, r1, omega)
    Ex2 = public_key.raw_encrypt(W2_coeffieicent)
    ...
    W2 = x2
    k1_2 =  W2
    k1_2_hat = Integrization(k1_2, r2, omega)
    k1_2_hat = (k1_2_hat * S2m) % omega
    W2_share_encrypted = ((pow(Ex2, k1_2_hat, N_square))) % N_square
    # it is ready to be sent to node 1. 
    ...
    W3_coeffieicent = c13
    # x_1_hat = Integrization(x_1, r1, omega)
    Ex3 = public_key.raw_encrypt(W3_coeffieicent)
    # send the coefficients to node 3


    # now it's time for node 3 to perform its tasks for the multiplication part
    W3 = x3
    k1_3 = W3
    k1_3_hat = Integrization(k1_3, r2, omega)
    k1_3_hat = (k1_3_hat * S3m) % omega
    W3_share_encrypted = ((pow(Ex3, k1_3_hat, N_square))) % N_square
     # it is ready to be sent to node 1. 
    ...
    W2_share_plain = private_key.raw_decrypt(W2_share_encrypted)
    ...
    W3_share_plain = private_key.raw_decrypt(W3_share_encrypted)
...
def grad(xx):
    ...
    F1 = 2 *a1 *(x1) + (2 * b11 * x1) + (b12 * x2 + b13 * x3 + b14 * x4) + (c11 * c12 * c13 * c14 ) * W

    F2 = 2 *a2 *(x2) + (2 * b22 * x2) + (b21 * x1 + b23 * x3 + b24 * x4) + (c21 * c22 * c23 * c24 ) * W

    F3 = 2 *a3 *(x3) + (2 * b33 * x3) + (b31 * x1 + b32 * x2 + b34 * x4) + (c31 * c32 * c33 * c34 ) * W

    F4 = 2 *a4 *(x4) + (2 * b44 * x4) + (b41 * x1 + b42 * x2 + b43 * x3) + (c41 * c42 * c43 * c44 ) * W
...
# plyer 1 parameters
a1 = 1; b11, b12, b13, b14 = (1, 1, 1, 1);  c11, c12, c13, c14 = (1, 1, 1, 1)

# plyer 2 parameter
a2 = 2; b21, b22, b23, b24 = (2, 2, 2, 2); c21, c22, c23, c24 = (1, 1, 1, 1)

# plyer 3 parameter
a3 = 3; b31, b32, b33, b34 = (3, 3, 3, 3); c31, c32, c33, c34 = (1, 1, 1, 1)

# plyer 3 parameter
a4 = 4; b41, b42, b43, b44 = (4, 4, 4, 4); c41, c42, c43, c44 = (1, 1, 1, 1)
